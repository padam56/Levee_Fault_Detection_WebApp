...
if processing_choice == "Image":
    ...
    if uploaded_image is not None:
        ...
        if sandboil_selected:
            ...
            sandboil_model = load_sandboil_model()  # Load Sandboil model lazily
            predictions = apply_model(sandboil_model, image, model_type="sandboil")

            if detection_choice == "Bounding Box":
                # Draw bounding boxes on the original image
                image_with_boxes = draw_bounding_boxes(image.copy(), predictions)
                st.image(image_with_boxes[..., ::-1], caption='Image with Bounding Boxes', use_column_width=True)
            else:
                # Overlay mode
                overlaid_image = overlay_mask_on_image(image, predictions, alpha=overlay_intensity, color=(0, 255, 0))
                st.image(overlaid_image[..., ::-1], caption='Image with Predicted Mask Overlay', use_column_width=True)

        # Seepage Detection
        elif seepage_selected:
            st.write("Selected Model: Seepage Detection")
            seepage_model = load_seepage_model()  # Load Seepage model lazily
            predictions = apply_model(seepage_model, image, model_type="seepage")
