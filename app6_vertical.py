...
if processing_choice == "Image":
    # Separate upload buttons for Sandboil and Seepage detection
    if sandboil_selected:
        ...
        if sandboil_image is not None:
            ...
            image = cv2.imdecode(np.frombuffer(sandboil_image.read(), np.uint8), cv2.IMREAD_COLOR)
            st.image(image[..., ::-1], caption="Uploaded Image for Sandboil Detection", use_column_width=True)
            ...
            overlaid_image = overlay_mask_on_image(image, sandboil_predictions, alpha=overlay_intensity, color=(0, 255, 0))
            st.image(overlaid_image[..., ::-1], caption='Sandboil Detection Overlay', use_column_width=True)
    ...
    if seepage_selected:
        ...
        if seepage_image is not None:
            # Read and decode uploaded image
            image = cv2.imdecode(np.frombuffer(seepage_image.read(), np.uint8), cv2.IMREAD_COLOR)
            st.image(image[..., ::-1], caption="Uploaded Image for Seepage Detection", use_column_width=True)
            ...
            # Overlay results on the image
            overlaid_image = overlay_mask_on_image(image, seepage_predictions, alpha=overlay_intensity, color=(255, 105, 180))
            st.image(overlaid_image[..., ::-1], caption='Seepage Detection Overlay', use_column_width=True)
