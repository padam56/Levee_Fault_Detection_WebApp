...
if processing_choice == "Image":
    ...
    with col1:
        if sandboil_selected:
            st.write("Upload an image for Sandboil Detection:")
            sandboil_image = st.file_uploader("Upload Sandboil Image", type=['jpg', 'png'], key="sandboil_upload")
            if sandboil_image is not None:
                # Read and decode uploaded image
                sandboil_image_cv = cv2.imdecode(np.frombuffer(sandboil_image.read(), np.uint8), cv2.IMREAD_COLOR)
                st.image(sandboil_image_cv[..., ::-1], caption="Uploaded Image for Sandboil Detection", use_column_width=True)

                # Load Sandboil model and apply detection
                st.write("Running Sandboil Detection...")
                sandboil_model = load_sandboil_model()
                sandboil_predictions = apply_model(sandboil_model, sandboil_image_cv, model_type="sandboil")

                # Overlay results on the image
                sandboil_overlay = overlay_mask_on_image(sandboil_image_cv, sandboil_predictions, alpha=overlay_intensity, color=(0, 255, 0))
                st.image(sandboil_overlay[..., ::-1], caption='Sandboil Detection Overlay', use_column_width=True)

    with col2:
        if seepage_selected:
            st.write("Upload an image for Seepage Detection:")
            seepage_image = st.file_uploader("Upload Seepage Image", type=['jpg', 'png'], key="seepage_upload")
            if seepage_image is not None:
                # Read and decode uploaded image
                seepage_image_cv = cv2.imdecode(np.frombuffer(seepage_image.read(), np.uint8), cv2.IMREAD_COLOR)
                st.image(seepage_image_cv[..., ::-1], caption="Uploaded Image for Seepage Detection", use_column_width=True)

                # Load Seepage model and apply detection
                st.write("Running Seepage Detection...")
                seepage_model = load_seepage_model()
                seepage_predictions = apply_model(seepage_model, seepage_image_cv, model_type="seepage")

                # Overlay results on the image
                seepage_overlay = overlay_mask_on_image(seepage_image_cv, seepage_predictions, alpha=overlay_intensity, color=(255, 105, 180))
                st.image(seepage_overlay[..., ::-1], caption='Seepage Detection Overlay', use_column_width=True)
