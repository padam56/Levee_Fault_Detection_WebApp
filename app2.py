...
def process_image(image):
    ...
    if edge_detection:
        ...
        image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR) # Convert back to BGR for display
    ...
    # Rotate the image by a specified angle if selected
    if rotate_angle != 0:
        (h, w) = image.shape[:2]
        center = (w // 2, h // 2)
...
if uploaded_image is not None:
    ...
    with tab1:
        ...
        # Convert OpenCV image (NumPy array) to PIL format for saving/downloading
        pil_image_to_save = Image.fromarray(processed_image_data_displayed.astype('uint8'))

        # Save feature: Download button for saving the processed file locally as PNG just below the displayed image
        buf = BytesIO()
        pil_image_to_save.save(buf, format="PNG")
        byte_im = buf.getvalue()

        # Adding download button using Streamlit's download_button widget directly below the displayed image.
        st.download_button(
    ...
    with tab2:
        ...
        # Display results using a horizontal bar chart
